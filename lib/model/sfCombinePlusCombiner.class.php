 ...
 */
abstract class sfCombinePlusCombiner
{
  /**
   * @var array
   */
   ...
   * @return  string
   */
  abstract protected function _getAssetPath($file);
  ...
  )
  {
    ...
    foreach ($this->getFiles() as $file)
    {
      ...
      $assetPath = $this->_getAssetPath($file);
      ...
      try {
        ...
        $filePath = sfCombinePlusUtility::getFilePath($fileName);
        ...
        sfContext::getInstance()->getLogger()->err(
            'sfCombinePlus exception: ' . $e->getMessage()
        );
      }
    }
   ...
   * @return  string
   */
  public function minify($content, $minifyMethod = array())
  {
    // get minify method from config
    $definedMinifyMethod = $this->getConfigOption('minify_method', false);

    if ($definedMinifyMethod && is_callable($definedMinifyMethod)) {
      $content = call_user_func($definedMinifyMethod, $content);
    } else if (is_callable($minifyMethod)) {
      $content = call_user_func($minifyMethod, $content);
    }
